...
# Python 学习 Day 2 发布计划（2026-09-13）

## 目标

在现有 Python 专题下新增承接 Day 1 的数据结构与函数基础文章，保持 Java、C++、Python 现有学习文章风格，并通过自动内容索引接入专题导航。

## 阶段

- [x] 复核 Python Day 1、Java/C++ 学习文章和专题入口
- [x] 确认 Day 2 主题、范围和文章结构
- [x] 编写实施计划
...
- Day 2 聚焦列表、元组、字典、集合、遍历、推导式、函数参数、返回值和作用域。
- 不在本篇提前深入面向对象、数据库、Web 或机器学习。
- 综合示例使用学习成绩统计，帮助复习 Day 1 的条件判断和循环。
- 继续使用自动生成 sidebar，不手动修改生成文件。
...
# C++ 学习 Day 3 文章计划（2026-09-15）

## 目标

在现有 C++ 专题下新增“数组、字符串、指针与引用”文章，承接 Day 2 的函数基础，为后续 STL、算法和项目实践打基础。

## 设计决策

- 采用基础原理与实际编程并重的方案：先解释 C 风格数组和指针，再优先使用 `std::string`、`std::array` 和 `std::vector`。
- 明确说明数组越界、空指针、野指针和引用失效等风险，但不在本篇深入动态内存和智能指针。
- 使用成绩统计作为综合示例，延续前两天“学习报告/学习数据”的上下文。
- 只新增正式文章并更新专题入口与 Day 2 下一步链接，继续依赖自动生成 sidebar。

## 阶段

- [x] 探索 C++ 专题结构、前两天文章和仓库状态
- [x] 确认 Day 3 设计并写入设计文档
- [ ] 编写 Day 3 文章并更新专题导航
- [ ] 生成索引、检查链接和 VuePress 构建
