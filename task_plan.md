...
# C++ 学习 Day 2 文章计划（2026-09-13）

## 目标

沿着 C++ 路线第 2 阶段，在现有专题下新增“函数、参数、返回值和作用域”学习文章，承接 Day 1 的变量、判断和循环，并保持 Java/Python 学习文章的目标、示例、练习和检查表风格。

## 设计决策

- 文章定位为第二天的可执行学习材料，不提前深入模板、类和复杂指针。
- 内容覆盖函数声明/定义/调用、返回值、值传递、引用参数、const 引用、局部/块/全局作用域、变量遮蔽、函数重载和头文件/源文件分离。
- 使用“学习进度报告”作为综合示例，把 Day 1 的长 main 拆成多个职责清楚的函数。
- README 和 Day 1 增加 Day 2 导航，路线文章保持阶段定义不变；自动 sidebar 继续由内容生成脚本维护。
...
| 错误 | 尝试 | 处理 |
|---|---:|---|
| 编译验证命令把 `\\n` 传成 C++ 多字符字面量 | 2 | 改用不含转义歧义的 `std::endl` 运行同等函数综合示例，输出总时长和平均时长正确 |
